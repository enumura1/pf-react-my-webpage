...
test('bottom container check words', () => {
  ...
  const checkTextElement2 = screen.getByText('・Process：CSS（box-shadow）生成ツールの制作を振り返って');
  const checkTextElement3 = screen.getByText('○sticker-memopad-tool（chrome拡張）');
  const checkTextElement4 = screen.getByText('・Process：chrome拡張機能 をリリースしたので開発過程を振り返ってみた');
